...
const isSymbol = c => isNaN(parseInt(c ?? ".")) && (c ?? ".") !== ".";
...
const numPos = [...grid.flatMap((line, y) =>
    line.flatMap((c, x) => isSymbol(c) &&
            calcAdj(x, y).map(c => findStartingNumber(grid, ...c))
    ).filter(c => Array.isArray(c))
).reduce((acc, c) => (acc.add(JSON.stringify(c)), acc), new Set()).values()].map(v => JSON.parse(v))

const sumOfJoined = numPos.map(c => getContigNumber(grid, ...c)).reduce((acc, v) => acc + v);

// console.log(contigMapping);
console.log(sumOfJoined);
