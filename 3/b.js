const input = `...................305.124................................432..............................................576..313.....514.................
...
const removeDup = a => [...a.map(n => JSON.stringify(n)).reduce((acc, v) => (acc.add(v), acc), new Set()).values()].map(n => JSON.parse(n));

const calcAdj = (x, y) => [-1,0,1].flatMap(a => [-1,0,1].map(b => [a, b]))
    .map(([ox, oy]) => [x + ox, y + oy]);

const findStartingNumber = grid => ([x, y]) => {
    const c = parseInt(grid[y]?.[x]);
...
const getContigNumber = grid => ([x, y]) => {
    const s = grid[y];
    let acc = "";
    for (;; x++)
...
const numPos = grid.flatMap((line, y) =>
    line.map((c, x) => isSymbol(c) &&
            removeDup(
                calcAdj(x, y)
                    .map(findStartingNumber(grid))
                    .filter(c => Array.isArray(c))
            )
    ).filter(a => a)
).filter(adj => adj.length === 2);

// console.dir(numPos, { depth: null })
const ratios = numPos.map(pairs =>
    pairs.map(getContigNumber(grid))
        .reduce((acc, v) => acc * v)
).reduce((acc, v) => acc + v);
