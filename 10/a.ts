...
const input = `F-|-J7F7F7F-7F7F--7-L|-L7777.FFL-|.|.FF-FF7FFFFF7--F-J7.|-FF77|-F|777-F7.F7FF|-J7|-7.FFJ77FL.7-|-|7.|.77|-F-|-F|-FL.F77.FF-77|7.7--7--7FJ--7
...
const mapRel = (relPos: RelPos): Offset => { switch (relPos) {
    case RelPos.North:
        return [0, -1]
    case RelPos.South:
        return [0, 1]
...
const pipeMapping: { [k: string]: [RelPos, RelPos] } = {
    "|": [RelPos.North, RelPos.South],
    "-": [RelPos.East, RelPos.West],
    "L": [RelPos.North, RelPos.East],
    "J": [RelPos.North, RelPos.West],
    "7": [RelPos.South, RelPos.West],
    "F": [RelPos.South, RelPos.East],
}

const getGridPos = (grid: Grid, x: number, y: number, pos: RelPos | null = null): Offset | null => {
    const relPos: Offset = pos !== null ? mapRel(pos) : [0, 0];
    const [tx, ty] = [x + relPos[0], y + relPos[1]];
    if (tx >= 0 && tx < grid[0].length && ty >= 0 && ty < grid.length)
        return [tx, ty]
    return null
}

const isConnectedTo = (grid: Grid, x: number, y: number, pos: RelPos) => {
    const currentPipe = grid[y][x]
    if (!currentPipe?.some(dir => dir === pos))
        return false
    const lookingPipe = getGridPos(grid, x, y, pos)
    // console.log([x, y], lookingPipe);
    return lookingPipe !== null &&
        grid[lookingPipe[1]][lookingPipe[0]]?.some(d => {
            // console.log(`looking ${getGridPos(grid, lookingPipe[0], lookingPipe[1], d)}`);
            return getGridPos(grid, lookingPipe[0], lookingPipe[1], d)?.join(" ") === [x, y].join(" ")
        })
}

const parseInput = (input: string): { startingPos: Offset, grid: Grid } => {
    let startingPos: Offset = [-1, -1];
    const grid = input.split("\n")
        .map((line, y) =>
            line.split("")
                .map((pipe, x) => {
                    if (pipe === "S") startingPos = [x, y]
                    return pipeMapping[pipe] ?? null
                })
        )
    for (const [p, dirs] of Object.entries(pipeMapping)) {
        grid[startingPos[1]][startingPos[0]] = dirs;
        if (dirs.every(d => isConnectedTo(grid, startingPos[0], startingPos[1], d)))
            break
    }
    ...
    do {
        flag = false
        // filter out only fully connected pipes (removes all dead ends)
        const gridOnlyValid = grid.map((a, y) => a.map((b, x) => b?.every(dir => {
            const c = isConnectedTo(grid, x, y, dir)
            if (c) flag = true
            return c
        }) ? b : null))
        return { startingPos, grid: gridOnlyValid }
    } while (flag)
    // console.log(grid);
}

const printGrid = (grid: Grid) => {
    console.log(grid.map(a => a.map(b => b === null ? "." : "O").join("")).join("\n"))
}

const gridFromPoints = (grid: Grid, path: Offset[]): Grid => {
    const loopRegion = new Set(path.map(p => p.join(" ")))
    return grid.map((c, y) => c.map((_, x) => loopRegion.has([x, y].join(" ")) ? [0, 0] : null))
}

const solvePath = (startingPos: Offset, grid: Grid) => {
    const queue = [[startingPos]]
    const visited = new Set<string>()
    let maxDist = -1;
    let maxDistPath: Offset[] = []
    while (queue.length > 0) {
        const lastPath = queue.pop()!
        const pos = lastPath[lastPath.length - 1]
        if (visited.has(pos.join(" "))) {
            if (pos.join(" ") === startingPos.join(" ") && lastPath.length > maxDist) {
                maxDist = lastPath.length
                maxDistPath = lastPath;
            }
            continue
        }
        visited.add(pos.join(" "))

        const pipeDirs = grid[pos[1]][pos[0]]
        if (!pipeDirs) continue

        const toPipes = pipeDirs.map(dir => getGridPos(grid, pos[0], pos[1], dir)!)
            .map(nextPos => [...lastPath, nextPos])
        queue.push(...toPipes)
    }
    return maxDistPath.slice(0, -1)
}

const solve = (grid: Grid, mainLoop: Offset[]) => {
    const invertPath = (path: Offset[]): Offset[] =>
        <Offset[]>gridFromPoints(grid, path).flatMap((a, y) => a.map((b, x) => b ? null : <Offset>[x, y]).filter(a => a !== null))

    // nonzero rule left->right
    // winding not zero indicates inside loop
    // literally the wikipedia description of the algorithm
    const nonZero = (points: Offset[], path: Offset[]) => {
        // interpolate path
        const directedPath = Object.fromEntries(path.map((pos, i) => {
            const next = path[i + 1] ?? path[0]
            const prev = path[i - 1] ?? path[path.length - 1]
            return [pos.join(" "), <Offset>[next[0] - prev[0], next[1] - prev[1]]]
        }))

        return points.filter(point => {
            let winding = 0

            let checkingPos: Offset | null = point
            do {
                if (checkingPos.join(" ") in directedPath) {
                    const relativeDirection = directedPath[checkingPos.join(" ")]
                    winding += relativeDirection[1]
                }
            } while ((checkingPos = getGridPos(grid, checkingPos[0], checkingPos[1], RelPos.East)) !== null);
            // console.log(point, winding)
            return winding !== 0
        })
    }

    // all points in grid that are not in main loop
    const enclosedPoints = invertPath(mainLoop)
    return nonZero(enclosedPoints, mainLoop)

}

const { startingPos, grid } = parseInput(input)
const path = solvePath(startingPos, grid)
const solution = solve(grid, path)

printGrid(gridFromPoints(grid, path))
printGrid(gridFromPoints(grid, solution));
