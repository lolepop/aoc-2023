...
const possibleCubeCount = {
    red: 12,
    green: 13,
    blue: 14
};

const parseRound = s => Object.fromEntries(
    s.split(", ").map(v =>
        [v.split(" ")[1], parseInt(v.split(" ")[0])]
    )
);

const isCubeCountPossible = possibleCubeCount => round =>
    Object.entries(round).every(([colour, drawn]) =>
        possibleCubeCount[colour] >= drawn
    );

const minPossibleCount = rounds =>
    rounds.reduce((acc, round) =>
        Object.fromEntries(
            Object.entries(acc).map(([colour, count]) =>
                [colour, Math.max(count, round[colour] ?? 0)]
            )
        )
...
const games = input.split("\n")
    ...
    .map(s =>
        s.split("; ")
            .map(parseRound)
            // .every(isCubeCountPossible(possibleCubeCount))
    )
    .map(minPossibleCount)
    .map(roundCounts =>
        Object.values(roundCounts).reduce((acc, v) =>
            acc * v
        )
    )
    .reduce((acc, v) => acc + v)
