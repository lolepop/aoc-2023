...
const INPUT: &str = "vdm{m<3285:R,R}
...
#[derive(Clone, Debug)]
enum Token {
    Conditional((Operator, String, isize), Box<Token>, Box<Token>),
    Branch(String),
    Accept,
    Reject
}

fn parse(input: &str) -> (HashMap<&str, Token>, Vec<HashMap<&str, isize>>) {
    fn parse_params(s: &str) -> Token {
        if let Some((condition, tail)) = s.split_once(":") {
            let (a, b) = tail.split_once(",").unwrap();
            let condition = if let Some((a, b)) = condition.split_once("<") {
                (Operator::Lt, a.to_string(), b.parse().unwrap())
            } else if let Some((a, b)) = condition.split_once(">") {
                (Operator::Gt, a.to_string(), b.parse().unwrap())
            } else {
                unimplemented!()
            };
            Token::Conditional(condition, Box::new(parse_params(a)), Box::new(parse_params(b)))
        } else {
            match s {
                "A" => Token::Accept,
                "R" => Token::Reject,
                s => Token::Branch(s.to_string())
            }
        }
    }

    let (conditions, params) = input.split_once("\n\n").unwrap();
    let conditions = conditions.lines()
        .map(|p| {
            let (name, c) = p.split_once("{").unwrap();
            let c = parse_params(&c[..c.len()-1]);
            (name, c)
        })
        .collect::<HashMap<_, _>>();
    // println!("{:?}", conditions);
    let params = params.lines().map(|p|
        p[1..p.len()-1].split(",")
            .map(|a| a.split_once("=").unwrap())
            .map(|(a, b)| (a, b.parse::<isize>().unwrap()))
            .collect::<HashMap<_, _>>()
    ).collect::<Vec<_>>();
...
fn solve_a(condition: &HashMap<&str, Token>, params: &HashMap<&str, isize>) -> bool {
    fn inner(token: &Token, condition: &HashMap<&str, Token>, params: &HashMap<&str, isize>) -> bool {
        match token {
            Token::Conditional((o, k, v), a, b) => {
                let n = params.get(k.as_str()).unwrap();
                let r = match o {
                    Operator::Gt => n > v,
                    Operator::Lt => n < v,
                };
                let r = if r { a } else { b };
                inner(r, condition, params)
            },
            Token::Branch(n) => inner(condition.get(n.as_str()).unwrap(), condition, params),
            Token::Accept => true,
            Token::Reject => false,
        }
    }

    let start = condition.get("in").unwrap();
    inner(&start, condition, params)
}

fn solve_b(condition: &HashMap<&str, Token>) -> isize {
    fn m(s: &str) -> usize {
        match s {
            "x" => 0,
            "m" => 1,
    ...
    fn cm(params: &[(isize, isize); 4], ki: usize, m: (isize, isize)) -> [(isize, isize); 4] {
        let mut tmp = params.clone();
        tmp[ki] = m;
        tmp
    }

    fn inner(token: &Token, condition: &HashMap<&str, Token>, params: &[(isize, isize); 4]) -> isize {
        match token {
            Token::Conditional((o, k, v), a, b) => {
                let ki = m(k);
                let (start, end) = params[ki];
                if start > end { unreachable!() }
                // try with range that satisfies either truthy/falsy condition
                let mut total = 0_isize;
                match o {
                    Operator::Gt => {
                        if start < *v && end > *v {
                            total += inner(a, condition, &cm(params, ki, (v + 1, end)));
                        }
                        if start <= *v && end > *v {
                            total += inner(b, condition, &cm(params, ki, (start, *v)));
                        }
                    },
                    Operator::Lt => {
                        if start < *v && end > *v {
                            total += inner(a, condition, &cm(params, ki, (start, v - 1)));
                        }
                        if start < *v && end >= *v {
                            total += inner(b, condition, &cm(params, ki, (*v, end)));
                        }
                    },
                };
                total
            },
            Token::Branch(k) => inner(condition.get(k.as_str()).unwrap(), condition, params),
            Token::Accept => params.iter().map(|v| v.1 - v.0 + 1).reduce(|acc, v| acc * v).unwrap(),
            Token::Reject => 0,
        }
    }

    let initial = [(1_isize, 4000_isize); 4];
    let start = condition.get("in").unwrap();
    inner(&start, condition, &initial)
}

fn main() {
    let (conditions, params) = parse(INPUT);
    let solution = params.iter().filter(|p|
        solve_a(&conditions, p)
    ).fold(0, |acc, p|
        acc + p.iter().fold(0, |acc, (_, a)| acc + a)
    );
    println!("{solution}");

    let solution = solve_b(&conditions);
    println!("{solution}");
}
